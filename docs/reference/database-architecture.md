...
embedding: vector(1024) (当前检索链路使用的 embedding 向量)
...
当前 schema 里的向量列固定为 `vector(1024)`。这意味着无论 embedding provider 是 Google 还是 Cloudflare，实际使用的 `EMBEDDING_MODEL` 都必须和 `EMBEDDING_DIMENSIONS=1024` 对齐，否则检索链路会在服务端快速失败，而不是把错误留到 SQL RPC 才暴露。

这里的“可切换”是受控切换，不是运行时热切换。要更换向量维度，至少需要同时完成：

- 更新服务端配置里的 `EMBEDDING_DIMENSIONS`
- 执行数据库 migration，把 `node_embeddings.embedding`、相关 RPC 签名与索引切到新维度
- 清空或重建旧的 `node_embeddings`
- 重新做一次全量 embedding backfill
